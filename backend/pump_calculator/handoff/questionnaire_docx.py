...
def generate_questionnaire_docx(
    ...
    *,
    ...
    _add_field_table(doc, [
        ...
        ("DISPATCHING", "Диспетчеризация (GSM / Modbus_RTU / Modbus_TCP / Ethernet / нет)", ""),
        ("CONTROL_TYPE",
         "Тип управления (поплавки / гидростат / радар / ультразвук)",
         ""),
        ("FLOAT_SWITCHES_QTY",
         "Количество поплавков (4 = типовое: нижн.стоп / нижн.старт / верхн.старт / аварийн.)",
         ""),
        ("PUMP_LOGIC",
         "Логика работы (alternation / separate_start_common_stop / load_share_VFD)",
         ""),
        ("CIM200_MODBUS_MODULE",
         "Модуль Grundfos CIM 200 для Modbus RTU (да / нет)",
         ""),
    ])

    # ---------- 7. Климат и площадка (Phase 20: КС-14) ----------
    _add_section_heading(doc, "7. Климат и площадка")
    _add_field_table(doc, [
        ("AMBIENT_TEMP_MIN_C", "Минимальная температура воздуха, °C", ""),
        ("AMBIENT_TEMP_MAX_C", "Максимальная температура воздуха, °C", ""),
        ("COLDEST_5DAY_TEMP_C",
         "Температура наиболее холодной пятидневки, °C (СП 131.13330, 0.92 обесп.)",
         ""),
        ("FROST_DEPTH_MM",
         "Глубина мерзлой почвы, мм (1700-1900 для Казахстана/Якутии)",
         ""),
        ("SEISMIC_MAGNITUDE",
         "Сейсмичность площадки, баллы (СП 14.13330, ОСР-2015)",
         ""),
        ("ABOVE_GROUND_PAVILION",
         "Наземный павильон с отоплением (да / нет)",
         ""),
        ("UGV_BELOW_BOTTOM_M",
         "Расстояние от дна корпуса до УГВ, м (если <1 — нужен пригруз)",
         ""),
    ])

    # ---------- 8. Подъём и обслуживание (КС-14) ----------
    _add_section_heading(doc, "8. Подъём и обслуживание")
    _add_field_table(doc, [
        ("HOIST_REQUIRED", "Грузоподъёмное устройство — таль (да / нет)", ""),
        ("HOIST_CAPACITY_T",
         "Грузоподъёмность тали, т (1.0 типовое для КНС, 2-5 для крупных)",
         ""),
        ("GUIDE_RAILS_MATERIAL",
         "Материал направляющих (StZn / AISI304 / AISI316 / duplex)",
         ""),
        ("LIFTING_CHAIN_MATERIAL",
         "Материал цепи подъёма (StZn / AISI304 / AISI316)",
         ""),
        ("SERVICE_LIFE_YEARS",
         "Заявленный срок службы, лет (25 типовое, 30 газпром, 50 морские)",
         ""),
    ])

    # ---------- 9. Трасса (расширено для КС-14) ----------
    _add_section_heading(doc, "9. Трасса (расширенно)")
    _add_field_table(doc, [
        ("PIPE_BENDS_COUNT",
         "Количество поворотов 90° на напорной трассе",
         ""),
        ("DZ_GEODETIC_M",
         "Геодезическая разность высот ΔZ, м (только подъём, без потерь)",
         ""),
        ("FLOWMETER_REQUIRED", "Расходомер на трассе (да / нет)", ""),
        ("FLOWMETER_LOCATION",
         "Расположение расходомера (pressure_pipe / gravity_pipe / per_pump)",
         ""),
        ("SUPPLY_PIPE_SPEC_GOST",
         "Спецификация подводящей трубы (ГОСТ Р 54475-2011 PE SN8 DN160)",
         ""),
        ("DISCHARGE_PIPE_SPEC_GOST",
         "Спецификация напорной трубы (PE100 SDR17 СТ РК ISO 4427-2014 DN90 PN16)",
         ""),
    ])

    # ---------- 10. Резервуары и доочистка (если применимо) ----------
    _add_section_heading(doc, "10. Резервуары и доочистка (если применимо)")
    _add_field_table(doc, [
        ("UV_DISINFECTION_REQUIRED", "УФ-обеззараживание (да / нет)", ""),
        ("UV_THROUGHPUT_M3H", "Производительность УФ-блока, м³/ч", ""),
        ("UV_DOSE_MJ_CM2",
         "Доза УФ облучения, мДж/см² (минимум 25 для питьевой)",
         ""),
        ("TANK_CAPACITY_M3",
         "Объём резервуара питьевой/технической воды, м³",
         ""),
        ("TANK_STEEL_GRADE",
         "Марка нержавейки (08X18N10 / AISI304 / AISI316 / 12X18N10T)",
         ""),
        ("TANK_THERMAL_INSULATION",
         "Утепление резервуара (да / нет)",
         ""),
    ])

    # ---------- 11. Прочее ----------
    _add_section_heading(doc, "11. Прочие требования")
    _add_field_table(doc, [
